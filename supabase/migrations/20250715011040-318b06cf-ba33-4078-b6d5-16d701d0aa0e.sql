...
CREATE INDEX IF NOT EXISTS idx_availability_bookings_user_instructor ON public.availability_bookings(user_id, booking_type) WHERE booking_type IN ('training', 'course');
...
BEGIN
  ...
  WHERE ab.user_id = p_instructor_id
    AND ab.booking_type IN ('training', 'course')
