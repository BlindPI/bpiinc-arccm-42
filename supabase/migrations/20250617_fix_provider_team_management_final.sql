-- Fix Provider Team Management - Final Migration
-- This migration fixes all the issues with provider team management
...
-- Create ap_user_location_assignments table if it doesn't exist
...
DROP POLICY IF EXISTS "Users can view AP user location assignments" ON public.ap_user_location_assignments;
DROP POLICY IF EXISTS "SA and AD can manage AP user location assignments" ON public.ap_user_location_assignments;
...
BEGIN
    ...
    LEFT JOIN (
        ...
        GROUP BY ap.user_id, l2.id
    ) team_counts ON team_counts.user_id = aula.ap_user_id AND team_counts.location_id = aula.location_id
...
-- Add sample data if no AP user assignments exist and we have data
DO $$
BEGIN
    ...
    AND NOT EXISTS (SELECT 1 FROM public.ap_user_location_assignments) THEN
        ...
        ON CONFLICT (ap_user_id, location_id) DO NOTHING;
