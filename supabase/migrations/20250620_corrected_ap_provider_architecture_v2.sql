-- Corrected AP Provider Architecture Migration v2
-- Handles view dependencies properly before dropping columns
...
-- =============================================================================
-- DROP DEPENDENT VIEWS FIRST (FIXES THE ERROR)
-- =============================================================================

-- Drop all views that might depend on columns we want to change
DROP VIEW IF EXISTS unified_provider_assignments CASCADE;
DROP VIEW IF EXISTS unified_provider_assignments_view CASCADE;
DROP VIEW IF EXISTS provider_location_assignments CASCADE;
DROP VIEW IF EXISTS ap_user_assignments_view CASCADE;

RAISE NOTICE 'Dropped dependent views to allow column changes';
...
-- =============================================================================
-- SIMPLIFY AP_USER_LOCATION_ASSIGNMENTS (NOW SAFE TO DROP COLUMNS)
-- =============================================================================

-- Now we can safely drop the complex fields (no dependent views)
ALTER TABLE ap_user_location_assignments 
DROP COLUMN IF EXISTS assignment_role CASCADE,
DROP COLUMN IF EXISTS assignment_priority CASCADE,
DROP COLUMN IF EXISTS auto_assign_teams CASCADE,
...
-- =============================================================================
-- CREATE COMPATIBILITY VIEW FOR LEGACY SUPPORT
-- =============================================================================

-- Create a view that provides the legacy authorized_providers interface
...
WHERE p.role = 'AP' AND ala.status = 'active';
...
BEGIN
    ...
    RAISE NOTICE '=== CORRECTED AP PROVIDER ARCHITECTURE V2 COMPLETE ===';
    ...
    RAISE NOTICE '- Dropped dependent views BEFORE column changes';
...
VALUES (
    ...
    NOW(), 
    'Fixed view dependencies. AP User IS the Provider. Eliminated sync issues and Dashboard Integrity Panel errors.'
