-- Ultimate fix for crm_pipeline_stages table
-- This script handles all missing columns including pipeline_type
...
BEGIN
    ...
    -- Handle pipeline_type column - either add it or make it nullable
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'crm_pipeline_stages' 
        ...
        -- Make pipeline_type nullable or set a default
        ALTER TABLE crm_pipeline_stages ALTER COLUMN pipeline_type DROP NOT NULL;
        ALTER TABLE crm_pipeline_stages ALTER COLUMN pipeline_type SET DEFAULT 'sales';
    ELSIF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'crm_pipeline_stages' 
        AND column_name = 'pipeline_type'
    ) THEN
        -- Add pipeline_type column if it doesn't exist
        ALTER TABLE crm_pipeline_stages ADD COLUMN pipeline_type VARCHAR(20) DEFAULT 'sales';
    END IF;
END $$;
...
-- Insert the default pipeline stages with pipeline_type
INSERT INTO crm_pipeline_stages (stage_name, stage_description, stage_order, probability_percentage, stage_color, pipeline_type) VALUES
('Qualified Lead', 'Initial qualified lead with basic information', 1, 10, '#ef4444', 'sales'),
('Needs Analysis', 'Understanding specific training requirements', 2, 25, '#f97316', 'sales'),
('Proposal Sent', 'Formal proposal has been submitted', 3, 50, '#eab308', 'sales'),
('Negotiation', 'Discussing terms and finalizing details', 4, 75, '#22c55e', 'sales'),
('Closed Won', 'Successfully closed opportunity', 5, 100, '#10b981', 'sales'),
('Closed Lost', 'Opportunity was not successful', 6, 0, '#6b7280', 'sales');
...
SELECT 'Pipeline stages data:' as info;
SELECT id, stage_name, stage_description, stage_order, probability_percentage, stage_color, pipeline_type, is_active
FROM crm_pipeline_stages 
ORDER BY stage_order;

-- Show success message
SELECT 'CRM Pipeline Stages table successfully configured with all required columns!' as status;