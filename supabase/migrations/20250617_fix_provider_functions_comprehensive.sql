-- Comprehensive fix for provider location functions
-- Addresses all column reference and type mismatch issues
...
-- Fix all column references and type mismatches
...
BEGIN
  ...
  WHERE ap.id = p_provider_id  -- This is INTEGER = INTEGER, no type mismatch
...
COMMENT ON FUNCTION get_provider_location_kpis(INTEGER) IS 'Get KPI metrics for a specific provider location - comprehensive fix for all column and type issues';
COMMENT ON FUNCTION get_provider_location_teams(INTEGER) IS 'Get team information for a specific provider location - comprehensive fix for all column and type issues';

-- Log the comprehensive fix
DO $$
BEGIN
  RAISE NOTICE 'Comprehensive provider location function fixes applied:';
  RAISE NOTICE '1. Dropped ALL versions (UUID, BIGINT, INTEGER) to prevent conflicts';
  RAISE NOTICE '2. Fixed certificates join: using instructor_name instead of non-existent instructor_id';
  RAISE NOTICE '3. Fixed performance_score: using teams.performance_score instead of non-existent tm.performance_score';
  RAISE NOTICE '4. Fixed type mismatches: ensured INTEGER = INTEGER comparisons';
  RAISE NOTICE '5. Added proper error handling and default values';
  RAISE NOTICE '6. Only INTEGER parameter versions remain to prevent overloading issues';
END;
$$;