-- Fix column reference errors in provider location functions
-- This migration corrects the SQL queries to reference existing columns
...
-- Remove references to non-existent courses and certificates tables
...
BEGIN
  ...
  SELECT 
    ...
    0::INTEGER as total_courses, -- Placeholder since courses table doesn't exist
    0::INTEGER as certificates_issued, -- Placeholder since certificates table doesn't exist
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(t.performance_score::NUMERIC, 4.0) as performance_score -- Use teams.performance_score instead of team_members.performance_score
  FROM teams t
...
COMMENT ON FUNCTION get_provider_location_kpis(INTEGER) IS 'Get KPI metrics for a specific provider location - corrected column references';
COMMENT ON FUNCTION get_provider_location_teams(INTEGER) IS 'Get team information for a specific provider location - corrected column references';

-- Log the fix
DO $$
BEGIN
  RAISE NOTICE 'Provider location function column references fixed';
END;
$$;