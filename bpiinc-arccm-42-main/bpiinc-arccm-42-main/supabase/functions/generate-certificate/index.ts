  ...
  try {
    ...
    async function setField(fieldName, value, fontKey) {
      try {
        ...
        // Special handling for NAME field to adjust font size for long names
        if (fieldName === 'NAME') {
          ...
          if (nameLength > 25) {
            fontSize = 20; // Minimum size for very long names
          ...
          console.log(`Setting NAME field font size to ${fontSize} for name length ${nameLength}: "${value}"`);
