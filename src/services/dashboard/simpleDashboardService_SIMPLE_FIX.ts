...
export class SimpleDashboardService {
  ...
  static async getTeamMembers(teamId: string) {
    try {
      console.log('🔧 SIMPLE_FIX: Starting getTeamMembers for teamId:', teamId);
      
      // Get team members using simple query
      ...
      if (teamError) {
        console.error('🔧 SIMPLE_FIX: Error fetching team members:', teamError);
        throw new Error(`Failed to get team members: ${teamError.message}`);
      }

      if (!teamMembers || teamMembers.length === 0) {
        console.log('🔧 SIMPLE_FIX: No team members found for team:', teamId);
        return [];
      }

      console.log('🔧 SIMPLE_FIX: Found', teamMembers.length, 'active team members');

      // **SIMPLE FIX: Use direct profile queries that we know work**
      const result = await Promise.all(
        teamMembers.map(async (member) => {
          console.log('🔧 SIMPLE_FIX: Getting profile for user:', member.user_id);
          
          // Use the exact query pattern that works in SQL
          ...
          let displayName = `User ${member.user_id.substring(0, 8)}`;
          let email = '';
          let phone = '';
          let jobTitle = '';

          if (profileError) {
            console.warn('🔧 SIMPLE_FIX: Profile error for user', member.user_id, ':', profileError.message);
          } else if (profile && profile.display_name) {
            displayName = profile.display_name;
            email = profile.email || '';
            phone = profile.phone || '';
            jobTitle = profile.job_title || '';
            console.log('🔧 SIMPLE_FIX: Successfully got profile:', displayName);
          } else {
            console.warn('🔧 SIMPLE_FIX: Profile found but no display_name for user:', member.user_id);
          }
          ...
          if (currentUser.data.user) {
            ...
            if (userProfile?.role === 'AP') {
              ...
              if (apProvider?.primary_location_id) {
                ...
                console.log('🔧 SIMPLE_FIX: Applied location filter for AP user');
              }
            }
      ...
      console.log('🔧 SIMPLE_FIX: Final result:', result.map(r => `${r.display_name} (${r.roster_submissions} rosters)`));
      return result;
    } catch (error) {
      console.error('🔧 SIMPLE_FIX: Fatal error getting team members:', error);
      return [];
    }
  }
