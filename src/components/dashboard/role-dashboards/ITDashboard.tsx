...
import type { DatabaseUserRole } from '@/types/database-roles'; // Import DatabaseUserRole
 ...
 export function ITDashboard() {
   ...
   useEffect(() => {
     // Check if user and profile are loaded and if it's an IT user
     if (!isProfileLoading && user && userProfile && userProfile.role === 'IT') {
       loadDashboardData(user.id, userProfile.role as 'IT' | 'IP' | 'IC' | 'AP');
     } else if (!isProfileLoading && user && userProfile && userProfile.role !== 'IT') {
       toast.error(`Access Denied: You are logged in as ${userProfile.role}, but this dashboard is for IT users.`);
       setIsLoading(false);
     } else if (!isProfileLoading && !user) {
       setIsLoading(false); // User not logged in
       toast.info("Please log in as an IT user to view this dashboard.");
     }
   ...
   const loadDashboardData = async (userId: string, role: 'IT' | 'IP' | 'IC' | 'AP') => {
     ...
     try {
       ...
       console.error('Error loading IT dashboard data:', error);
       ...
       <Card className="w-full">
         ...
         <CardContent>
           <div className="flex items-center gap-2">
             ...
             Loading IT Compliance Dashboard for {userProfile?.display_name || user?.email || 'user'}...
           </div>
         </CardContent>
       </Card>
     );
   ...
   if (userProfile.role !== 'IT') { // This handles access control.
     return (
       <Card className="w-full">
         <CardHeader>
           <CardTitle className="flex items-center gap-2">
         ...
         <CardContent>
           ...
           <AlertDescription>
             Access Denied: This dashboard is only for Instructor Trainee (IT) users.
           </AlertDescription>
         </CardContent>
       </Card>
     );
  ...
  // Helper to get badge style for compliance status
    ...
    <div className="space-y-6">
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <ShieldCheck className="h-5 w-5" />
            IT Compliance Dashboard - {userProfile.display_name || user?.email}
          </CardTitle>
          <CardDescription>
            Overview of your compliance status as an Instructor Trainee.
          </CardDescription>
        </CardHeader>
            ...
            <div className="space-y-4">
              {userRecords.map(record => (
                <div key={record.id} className="border p-3 rounded-md shadow-sm">
                  <div className="font-semibold">{record.compliance_metrics?.name}</div>
