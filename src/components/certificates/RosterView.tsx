...
export function RosterView({ certificates, isLoading }: RosterViewProps) {
  ...
  const [selectedBatchCertificates, setSelectedBatchCertificates] = useState<any[]>([]);
  ...
  const handleBatchEmail = async (batchId: string) => {
    ...
    setSelectedBatchCertificates(batchCertificates);
  ...
  return (
    <>
      ...
      {/* Use Dialog component directly in the JSX hierarchy to ensure it's within all providers */}
      <Dialog 
        open={batchEmailDialogOpen} 
        onOpenChange={setBatchEmailDialogOpen}
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Send Certificate Emails</DialogTitle>
          </DialogHeader>
          {batchEmailDialogOpen && (
            <BatchCertificateEmailForm
              certificateIds={selectedBatchCertificates.map(cert => cert.id)}
              certificates={selectedBatchCertificates}
              onClose={() => setBatchEmailDialogOpen(false)}
            />
          )}
        </DialogContent>
      </Dialog>
