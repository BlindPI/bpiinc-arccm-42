...
export function BatchCertificateUpload() {
  ...
  const processFileContents = async (file: File) => {
    ...
    try {
      const rows = file.name.toLowerCase().endsWith('.xlsx') ? await processExcelFile(file) : await processCSVFile(file);
      setProcessingStatus({
        total: rows.length,
        processed: 0,
        successful: 0,
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input id="file" type="file" accept=".csv,.xlsx" onChange={handleFileUpload} disabled={isUploading || !selectedCourseId || !issueDate || !isValidated} />
            </div>
