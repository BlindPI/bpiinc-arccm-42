      ...
      try {
        ...
        if (mounted) {
          ...
          if (initialSession?.user) {
            try {
              ...
              // Create minimal user object for fallback
              setUser({
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange(
          (event, session) => {
            ...
            if (session?.user) {
              // Defer async profile fetching to avoid performance issues
              setTimeout(() => {
                if (mounted) {
                  getUserWithProfile(session.user).then(userWithProfile => {
                    ...
                    console.error('🔍 DEBUG: useAuthInit - Error getting user profile in state change:', error);
                    if (mounted) {
